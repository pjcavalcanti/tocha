...
class TestLSTMCell(unittest.TestCase):
    def test_rnn_against_torch_output_0(self):
        ...
        for _ in range(100):
            ...
            lstm_man.i_weight_ih.data = weight_ih[:hidden_size, :].t().detach().numpy().copy()
            lstm_man.i_weight_hh.data = weight_hh[:hidden_size, :].t().detach().numpy().copy()
            lstm_man.f_weight_ih.data = weight_ih[hidden_size : 2 * hidden_size, :].t().detach().numpy().copy()
            lstm_man.f_weight_hh.data = weight_hh[hidden_size : 2 * hidden_size, :].t().detach().numpy().copy()
            lstm_man.g_weight_ih.data = weight_ih[2 * hidden_size : 3 * hidden_size, :].t().detach().numpy().copy()
            lstm_man.g_weight_hh.data = weight_hh[2 * hidden_size : 3 * hidden_size, :].t().detach().numpy().copy()
            lstm_man.o_weight_ih.data = weight_ih[3 * hidden_size :, :].t().detach().numpy().copy()
            lstm_man.o_weight_hh.data = weight_hh[3 * hidden_size :, :].t().detach().numpy().copy()
            if bias:
                lstm_man.i_bias_ih.data = bias_ih[:hidden_size].t().detach().numpy().copy()
                lstm_man.i_bias_hh.data = bias_hh[:hidden_size].t().detach().numpy().copy()
                lstm_man.f_bias_ih.data = bias_ih[hidden_size : 2 * hidden_size].t().detach().numpy().copy()
                lstm_man.f_bias_hh.data = bias_hh[hidden_size : 2 * hidden_size].t().detach().numpy().copy()
                lstm_man.g_bias_ih.data = bias_ih[2 * hidden_size : 3 * hidden_size].t().detach().numpy().copy()
                lstm_man.g_bias_hh.data = bias_hh[2 * hidden_size : 3 * hidden_size].t().detach().numpy().copy()
                lstm_man.o_bias_ih.data = bias_ih[3 * hidden_size :].t().detach().numpy().copy()
                lstm_man.o_bias_hh.data = bias_hh[3 * hidden_size :].t().detach().numpy().copy()
